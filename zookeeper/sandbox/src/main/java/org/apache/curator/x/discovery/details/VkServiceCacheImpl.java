...
public class VkServiceCacheImpl<T> implements ServiceCache<T>, PathChildrenCacheListener {
    ...
    public VkServiceCacheImpl(ServiceDiscoveryImpl<T> discovery, String name, ThreadFactory threadFactory) {
        this(discovery, name, convertThreadFactory(threadFactory));
    }

    VkServiceCacheImpl(ServiceDiscoveryImpl<T> discovery, String name, ExecutorService executorService) {
    ...
    public void close() {
        Preconditions.checkState(
                state.compareAndSet(State.STARTED, State.STOPPED), "Already closed or has not been started");

        listenerContainer.forEach(
                l -> discovery.getClient().getConnectionStateListenable().removeListener(l));
